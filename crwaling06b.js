...
// movie.daum.net 사이트에서 '상영중인 영화'에 대한 정보를 끍어오기
...
async  function main() { // 비동기 I/O 지원 함수 정의
    ...
    try {
       ...
       for (let movie of movies){
           //console.log(await movie.getText()) ; //getText() 눈에 보이는 요소의 텍스트만 출력
       ...
       // 추출된 평점 출력
        for (let rate of rates){
            let rateAll = await rate.getAttribute('textContent');
           // console.log(rateAll);
            ratess.push(rateAll);
        };
        ...
        for (let rsrv of rsrvs){
            let rsrvAll = await rsrv.getAttribute('textContent');
            //console.log(rsrvAll);
            rsrvss.push(rsrvAll);
        };
